<template>
  <DmCard :loading="loading" class="chartMap" title="CC攻击统计地图" style="margin-top: 12px">
    <template slot="action">
      <el-radio-group v-model="activeProvince" @change="init">
    ...
    <el-row :gutter="12">
      ...
      <el-col :span="10">
        <yd-table style="margin-top: 12px" type="white">
          <el-table :data="chartProvince.data.rows.slice(0, 10)">
            <el-table-column type="index" label=" " width="50" />
            <el-table-column :label="activeProvince === 'china' ? '省份' : '国家'" prop="name" />
            <el-table-column label="攻击次数" align="right" width="100">
              <template slot-scope="scope">{{ scope.row.value | formatNumber }} 次</template>
            </el-table-column>
            <el-table-column label="占比" prop="percent" width="120">
...
export default {
  ...
  methods: {
    async fetchData(params) {
      this.loading = true;
      const data =
        this.activeProvince === 'china'
          ? await this.Fetch.post(
              'V4/statistic.tjkd.app.tcp.cc.Province.top',
              params
            )
          : await this.Fetch.get(
              'V4/statistic.tjkd.app.tcp.cc.Country.top',
              params
            );
